...
- `clarify` — 向用户提问澄清。当用户需求模糊、有多种合理解读，或缺少关键信息（如研究方向不明、对比对象不清、时间范围未定）导致无法可靠开展检索时，先用它向用户确认。参数 question 是要确认的问题，choices 是可选项（最多 4 项，可不传）。调用后会暂停等待用户回答。不要为已经清晰的需求反复发问；能直接合理推断时优先直接执行
